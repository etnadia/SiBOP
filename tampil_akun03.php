	...
	<div class="panel panel-primary">
		...
		<div class="panel-heading">
			...
			while ($p = mysqli_fetch_array($sql)) {
				...
				echo "<h3 class='panel-title'>KEGIATAN  LAINNYA</h3>";
		...
		<div class="panel-body">
			<div>
				<a href="tambah_akun03.php" class="btn btn-success" style="margin-bottom: 10px;">
			...
			<table class="table">
                ...
                <td>" . buatrp($d['hrgsatuan']) . "</td>
								...
								<td>$d[waktu]</td>
								<td>
									<a href='edit_akun03.php?id=$d[idakun]'
									   class='btn btn-warning btn-xs'>Edit</a>  
									<a href='hapus_akun3.php?id=$d[idakun]' class='btn btn-danger btn-xs'>Hapus</a>
								</td>
							 </tr>";
					$no++;
				}
				$sal = mysqli_query(
					$konek,
					"SELECT sum(totharga) as grandtotal 
						 FROM akun
			       WHERE kegutama like 'KEGIATAN LAINNYA'
						 ORDER BY kodeakun"
				);
				$sql = mysqli_query(
					$konek,
					"SELECT *,(jmlsiswa * danasiswa) as danabop
						 FROM profile"
				);
				$e   = mysqli_fetch_array($sql);
				$sld = mysqli_query(
					$konek,
					"SELECT sum(totharga) as grand 
						 FROM akun 
						 ORDER BY kodeakun"
				);
				...
				while ($saldo = mysqli_fetch_array($sal)) {
					...
					echo "<tr class='alert alert-info'>
								<td></td><td></td><td colspan='2'><b>Total Anggaran Kegiatan Lainnya Rp. </b></td><td></td>
			...
			<div class="alert alert-danger" role="alert">
				...
				echo "<b>KEGIATAN LAINNYA MAKS. 15% : Rp." . $maks . "</b>";
				?>
			</div>
		</div>
	</div>
