	...
	<div class="panel panel-primary">
		...
		<div class="panel-heading">
			...
			while ($p = mysqli_fetch_array($sql)) {
				...
				echo "<h3 class='panel-title'><center>KEGIATAN PEMBELAJARAN DAN BERMAIN</center></h3>";
				echo "<h2 class='panel-title'><center>$paud</center></h2>";
				echo "<h3 class='panel-title'><center>Periode Pelaporan: $tgl</center></h3>";
			}
			?>
		</div>
		<div class="panel-body">
			<div>
				<a href="tambah_akun.php" class="btn btn-success" style="margin-bottom: 10px;">
			...
			<table class="table">
				...
				$sql = mysqli_query($konek, "SELECT * FROM akun 
																	WHERE kegutama like 'KEGIATAN PEMBELAJARAN DAN BERMAIN' 
																	ORDER BY kodeakun");
				$no = 1;
                ...
                <td>" . buatrp($d['hrgsatuan']) . "</td>
								...
								<td>$d[waktu]</td>
								<td>
									<a href='edit_akun.php?id=$d[idakun]'
									   class='btn btn-warning btn-xs'>Edit</a>  
									<a href='hapus_akun.php?id=$d[idakun]' class='btn btn-danger btn-xs'>Hapus</a>
								</td>
							 </tr>";
					$no++;
				}
				$sal = mysqli_query(
					$konek,
					"SELECT sum(totharga) as grandtotal FROM akun 
													 WHERE kegutama like 'KEGIATAN PEMBELAJARAN DAN BERMAIN'
													 ORDER BY kodeakun"
				);
				$sql = mysqli_query(
				...
				while ($saldo = mysqli_fetch_array($sal)) {
					...
					$danabop  = $e['danabop'];
					$vmaks    = $danabop * 0.5;
					$persen   = ($grand / $vmaks) * 100;
					$kurang   = $vmaks - $saldo['grandtotal'];
					echo "<tr class='alert alert-info'>
								<td></td><td></td><td colspan='2'><b>Total Anggaran Pembelajaran Dan Bermain Rp. </b></td>
								<td></td><td></td><td></td><td><b>" . buatrp($saldo['grandtotal']) . "</b></td>
			...
			<div class="alert alert-danger" role="alert">
				...
				echo "<b>PENGGUNAAN DANA PEMBELAJARAN DAN BERMAIN MIN. 50% : Rp." . $maks . "</b>";
				?>
			</div>
		</div>
	</div>
